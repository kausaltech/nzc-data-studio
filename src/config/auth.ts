...
function accessTokenNeedsRefresh(token: JWT) {
  ...
  if (expiresIn < 0) {
    authLogger.info(
      `Token expired at ${new Date(expiresAt * 1000).toISOString()}`
    );
    return true;
  } else {
    authLogger.debug(
      `Token will expire at ${new Date(expiresAt * 1000).toISOString()}`
    );
  }
...
export async function refreshAccessToken(token: JWT) {
  const refreshToken =
    token.refreshToken ?? (token.refresh_token as string | undefined);
...
const authConfig: NextAuthConfig = {
  callbacks: {
    async jwt(resp) {
      const authLogger = getAuthLogger();
      authLogger.debug(
        `${process.env.NEXT_RUNTIME}: JWT callback ${Object.getOwnPropertyNames(resp).join(', ')}`
      );
      ...
      const updatedToken: JWT = {
        ...
        accessTokenLastCheckedAt:
          (profile?.iat as number | undefined) ?? Date.now() / 1000,
        user: {
          name: user.name,
          email: null,
  ...
  logger: {
    ...
    debug(_message, _metadata) {
      const authLogger = getAuthLogger();
      authLogger.info(_metadata, _message);
      return;
    },
    warn(code) {
      const authLogger = getAuthLogger();
