...
const authConfig: NextAuthConfig = {
  callbacks: {
    async jwt(resp) {
      authLogger.debug(`${process.env.NEXT_RUNTIME}: JWT callback ${Object.getOwnPropertyNames(resp)}`);
...
export const { handlers, signIn, signOut, auth, unstable_update } = NextAuth((req?: NextRequest) => {
  return authConfig;
});
